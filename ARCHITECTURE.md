...
We've now built version â‘¢ and are moving towards version â‘£. Right now, everything is in a single repository, and the solution will need to be split into libraries and published in parts to npm. Meanwhile, we'll write the UI and business logic.
...
- See Future features section for planned enhancements: voice/video calls, AI integration, bots, and more UI solution: terminal-like text interface with modern controls.
...
- 12:41  timur: message textâ€¦
- Reactions aligned: :+1:3 :fire:1
- Reply indicator: â†³ 5 replies (clock to see list)
- Forwarded content: â†ª from #dev (timur, 12:10): textâ€¦
